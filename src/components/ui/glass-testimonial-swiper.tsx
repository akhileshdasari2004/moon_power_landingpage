...
export const TestimonialStack = ({ testimonials, visibleBehind = 2 }: TestimonialStackProps) => {
  ...
  return (
    <section className="testimonials-stack relative pb-10 w-full max-w-lg mx-auto h-[400px] flex items-center justify-center">
      {testimonials.map((testimonial, index) => {
        ...
        const style: CSSProperties = {
          transition: isDragging && displayOrder === 0 ? 'none' : 'all 0.4s cubic-bezier(0.25, 1, 0.5, 1)',
          position: 'absolute',
          width: '100%',
        };
        ...
        return (
          <div
            ...
            key={testimonial.id}
            className="testimonial-card glass rounded-2xl shadow-2xl p-6 md:p-8 cursor-grab active:cursor-grabbing select-none"
          ...
          >
            <div>
              ...
              <blockquote className="text-foreground/90 leading-relaxed text-lg mb-6">"{testimonial.quote}"</blockquote>
      ...
      <div className="pagination flex gap-2 justify-center absolute -bottom-8 left-0 right-0">
        {testimonials.map((_, index) => (
          <button
            key={index}
            aria-label={`Go to testimonial ${index + 1}`}
            onClick={() => navigate(index)}
            className={`w-2 h-2 rounded-full transition-all duration-300 ${activeIndex === index ? 'bg-primary w-6' : 'bg-primary/30 hover:bg-primary/50'}`}
          />
        ))}
      </div>
    </section>
